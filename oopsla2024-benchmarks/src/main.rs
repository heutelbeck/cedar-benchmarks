...
fn run_experiment(
    ...
    (1..=common_args.num_hierarchies).for_each(|_| {
        ...
        // Generalized from the original 2/3/4-engine match to support any number of engines.
        // Logic is identical: compare all sequences against the first, panic on mismatch.
            ...
            for (req_idx, request) in requests.iter().enumerate() {
                ...
                for seq_idx in 1..decision_sequences.len() {
                    if decision_sequences[seq_idx][req_idx] != *first_decision {
                        // Print all decisions for context
